...
| マイルストーン運用 | **版だけを単位にする。** 名前は打つ予定のタグ名（`v0.1.0`）、**期限は置かない**。作るのは入れる issue が 1 本以上ある日、閉じるのは**タグを打った日**（`main` の作成・タグ・クローズは 1 続き）。付けるのは**本体に変更が入る issue と、版を切る操作**で、**PR には付けない**。無印は「**今の版に要らない**」——起票のたびに判断する。**リポジトリを跨ぐ束には使わない**——1 リポジトリしか数えず、本体分だけで 100% になる。**閉じたものにも遡って付ける。ただし一覧は完全にならない**（issue を持たない変更がタグに入る／本体に 1 行も入らないものは外す）——**版に入ったものの正本はタグ間の `git log`** で、ここは入口にとどまる。判断は #50 |
...
| Railway へ繋ぐリポジトリ | **`recibir-ops`（private、別リポジトリ）。本体は繋がない**——public な履歴に宛先アドレスが混入すると取り消せない。運用が要求する設定の切り替えは上の「最重要」が禁じている。**★ 渡すのはコードではなくコンテナイメージである**（#48。2026-08-30 に変わった）——**本体の CI**（`.github/workflows/image.yml`）が GHCR へ private イメージを push し、Railway がそれを引く。**`recibir-ops` はコードを 1 行も持たない。** 判断は #1 と #48 |
| `permissions`（`allow` ／ `ask` ／ `deny`） | **`allow` は `Bash(gh pr merge:*)` の 1 本だけ。`ask` と `deny` は置かない**——既定のプロンプトと規約で足りる。置き場所は **commit する側**（`.claude/settings.json`）で、1 回限りの許可だけを `.claude/settings.local.json` に残す。判断は #53 |
| permission mode | **既定。`acceptEdits` を常用しない**——ファイルシステム系の一部を自動承認するので、「既定に任せる」判断が成り立たなくなる |
| `allow` の棚卸しの契機 | **org の契機 ① を踏んだ次に、設定を触るとき。** 本数では決めない——守れているのに赤が出る検査になる |
...
| タグ注釈と Release に書くもの | **どちらも公開面で、同じ文を載せる**——Release の name がタグ注釈の 1 行目、body が残り。**連結してタグ注釈と 1 文字も違わないこと**（新しく書き下ろすと、その日から食い違いうる）。**版ごとに Release を 1 本作る**——**タグ注釈だけでは Releases タブも front page も空のままで、版が探す人に届かない**（2026-08-30 実測。`/tags` と `/releases/tag/<tag>` には出る）。**アセットは足さない**（#48）。**状態を書かない／作業の内部記録を書かない／版の中身を列挙しない**（正本は `git log`、版の差はタグ間で読む。#50）。**タグは追記で直せない**——直すには force で打ち直すことになる。判断は #58 / #61 |
