...
| タグ注釈に書くもの | **公開面である**——GitHub が `/tags` と `/releases/tag/<tag>` に**全文を描画する**（2026-08-30 実測。Release が 0 本でも後者は開き、`#46` のような番号は issue リンクになる）。**状態を書かない／作業の内部記録を書かない／版の中身を列挙しない**（正本はタグ間の `git log`。#50）。**タグは追記で直せない**——直すには force で打ち直すことになる。判断は #58 |
